...
#define CHECK(cond, fmt, ...) do { \
    if(!(cond)) { \
      fprintf(stderr, "implib-gen: libSDL2-2.0.so.0: " fmt "\n", ##__VA_ARGS__); \
...
static void *load_library() {
  ...
  lib_handle = ("libSDL2-2.0.so.0");
  CHECK(lib_handle, "failed to load library 'libSDL2-2.0.so.0' via callback ''");
#else
  lib_handle = dlopen("libSDL2-2.0.so.0", RTLD_LAZY | RTLD_GLOBAL);
  CHECK(lib_handle, "failed to load library 'libSDL2-2.0.so.0' via dlopen: %s", dlerror());
#endif
...
extern void *_libSDL2_so_tramp_table[];

// Can be sped up by manually parsing library symtab...
void _libSDL2_so_tramp_resolve(int i) {
...
#   endif
  ...
  _libSDL2_so_tramp_table[i] = (h, sym_names[i]);
  CHECK(_libSDL2_so_tramp_table[i], "failed to resolve symbol '%s' via callback ", sym_names[i]);
#else
  // Dlsym is thread-safe so don't need to protect it.
  _libSDL2_so_tramp_table[i] = dlsym(h, sym_names[i]);
  CHECK(_libSDL2_so_tramp_table[i], "failed to resolve symbol '%s' via dlsym: %s", sym_names[i], dlerror());
#endif
}

// Helper for user to resolve all symbols
void _libSDL2_so_tramp_resolve_all(void) {
  size_t i;
  for(i = 0; i < SYM_COUNT; ++i)
    _libSDL2_so_tramp_resolve(i);
}

// Allows user to specify manually loaded implementation library.
void _libSDL2_so_tramp_set_handle(void *handle) {
  lib_handle = handle;
  do_dlclose = 0;
}
...
void _libSDL2_so_tramp_reset(void) {
  memset(_libSDL2_so_tramp_table, 0, SYM_COUNT * sizeof(_libSDL2_so_tramp_table[0]));
  lib_handle = 0;
  do_dlclose = 0;
}
